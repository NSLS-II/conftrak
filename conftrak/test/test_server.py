...
def test_configuration_post(http_server, http_client, base_url):
    ...
    response = yield http_client.fetch(url, method="POST", body=body, raise_error=False)
...
def test_configuration_get(http_server, http_client, base_url):
    ...
    response = yield http_client.fetch(
        g_url, method="GET", body=body, raise_error=False
    )
    assert response.code == 200
...
def test_configuration_put(http_server, http_client, base_url):
    ...
    response = yield http_client.fetch(url, method="PUT", body=body, raise_error=False)
...
def test_schema_get(http_server, http_client, base_url):
    ...
    response = yield http_client.fetch(url, method="GET", body=body, raise_error=False)
    assert response.code == 200
